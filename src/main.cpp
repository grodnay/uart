...
class eth2uart
{
    ...
    {
      if (Serial1.availableForWrite() >= packet.length())
    ...
    void udp_send(void)
    {
      int availableForRead = Serial1.available();
      unsigned long int now = millis();
      if (availableForRead==0) { oldest_byte_time=now; return;}
...
void loop()
{
  uart1.update();
  uart2.update();
  if (millis()%1000==0) {Serial1.write("Hello1\n");Serial2.write("Hello2\n");}
}
